...
| B11 | Believes a child is present from the moment it asks the API server to create it, and never asks again. Every create the API server accepts makes the belief true, so no spec change reveals it. A create a fault refuses leaves the belief behind, and the toy stays one child short for good: no error, no requeue, no watch event | unconfirmed-write | G4 |
...
The matrix runs one `b<id>.json` sequence per bug, and none of them carries a fault
(§10, M3). B11's row is a `deleteManaged` op: the belief outlives the object, so the toy
reports one child short and never repairs it. What needs a fault is the unconfirmed write
itself, and `b11-fault.json` exercises that: §10 M6's acceptance test runs it. B11 is the
deterministic form of §5.6's "a transient state is made permanent by a `Fault`". The toy
goes quiet one child short of its spec, so the settle wait after the fault stops expires
whatever the windows are. A `Restart` heals it, because the belief lives in the process.
