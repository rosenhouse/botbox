...
type bugRow struct {
	...
	fired    []string
	// skipped names the checks that left a note, which a blank cell would read
	// as a pass (DESIGN.md §6, D31).
	skipped []string
}
...
and its row is empty. A `+"`✓`"+` caught the bug; a `+"`?`"+` judged nothing, which is not
the same as a pass.
