...
func TestParsePathRejects(t *testing.T) {
	for _, tc := range []struct {
		...
		{"status.conditions.*.message", []string{"offset 18", "[*]"}},
		{"data.a]b", []string{"offset 5", `["a]b"]`}},
		{`data.a"b`, []string{"offset 5", `["a\"b"]`}},
		{"[*].x", []string{"offset 0", "key"}},
		{"metadata.annotations.probe.example.com/started-at",
			[]string{"offset 21", `metadata.annotations["probe.example.com/started-at"]`}},
		...
		{"spec.selector.matchLabels.app.kubernetes.io/name", []string{"offset 41", "io/name", "whole key"}},
