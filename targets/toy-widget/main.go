...
func run(args []string) error {
	...
	probeAddress := flags.String("health-probe-bind-address", "0", "address the health probe server binds to; 0 disables it")
	...
	reconciler := &controller.Reconciler{Client: manager.GetClient(), Scheme: manager.GetScheme(), Bug: bug}
